...
%
% \footnote{\textbf{Important.}
% The package was improved a lot
% between version 0.1 to version 0.2. Unfortunately, due to some technical
% choice, bibliography written for version 0.2 are incompatible
% (because of few modification) with bibliography written for version 
% 0.1. If you prefer the old style you can find \texttt{easybib.sty} in the
% directory ``\texttt{old}''.}
%
...
The scheme for the bibliography in the package \package{easybib}
...
\end{verbatim}
The option \cmdname{definethebibliography} permits to override
the default \envname{thebibliography} environment.
In this case the environment \envname{thebibliography} take this form:
%
\begin{verbatim}
...
\end{verbatim}
%
so that you can override the default name \cmdname{refname} with your
own name, you can change the default \cmdname{section*} with
\cmdname{cmd} and add extra space \verb'space' before the items.  The
...
The command \cmdname{endref} ends the definition of a reference.
...
\cmdname{booklist}. For example in order to have the \verb'yr' item 
